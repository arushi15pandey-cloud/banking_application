    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl bg-[#131b2e] border border-slate-800">
        <div>
          ...
          <h1 className="text-2xl font-bold text-white tracking-tight">Transfers & Money Movements</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-6 rounded-3xl bg-[#131b2e] border border-slate-800 space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white placeholder:text-slate-500 focus:border-blue-500 focus:outline-none"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">
            ...
            <div>
              <div className="flex justify-between items-center mb-1.5">
                ...
                <span className="text-[11px] text-slate-400">
                  Available: <strong className="text-emerald-400">${availableBalance.toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong>
                </span>
              </div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-8 pr-4 py-3 bg-slate-900 border border-slate-800 rounded-xl text-base font-bold text-white placeholder:text-slate-600 focus:border-blue-500 focus:outline-none"
                />
              </div>
              ...
              <div className="flex items-center gap-2 mt-2">
                {[50, 100, 500, 1000, 2500].map(val => (
                  <button
                    ...
                    className="px-2.5 py-1 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-800 text-[11px] font-semibold text-slate-300 transition"
                  >
                    +${val}
                  </button>
                ))}
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold text-sm transition shadow-lg shadow-blue-900/40 flex items-center justify-center gap-2"
            >
              {statusState.loading ? (
        ...
        <div className="space-y-5">
            ...
            <div className="space-y-2.5">
              {frequentPayees.map((payee, idx) => (
                <div
                  ...
                  className="p-3 rounded-2xl bg-slate-900/80 border border-slate-800 hover:border-blue-500/40 cursor-pointer transition flex items-center justify-between group"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-xl bg-blue-500/10 text-blue-400 font-bold text-xs flex items-center justify-center border border-blue-500/20">
                      {payee.avatar}
                    </div>
                    <div>
                      <p className="text-xs font-semibold text-slate-200 group-hover:text-blue-400 transition">{payee.name}</p>
                      <p className="text-[10px] text-slate-400">{payee.bank} • {payee.account}</p>
                    </div>
                  </div>
                  <Send className="w-3.5 h-3.5 text-slate-500 group-hover:text-blue-400 transition" />
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="p-5 rounded-3xl bg-slate-900/60 border border-slate-800 space-y-3">
            <div className="flex items-center gap-2 text-emerald-400 text-xs font-semibold">
              <ShieldCheck className="w-4 h-4" /> Apex Guarantee
            </div>
      ...
      {statusState.receipt && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4 animate-fade-in">
          <div className="w-full max-w-md bg-[#131b2e] border border-slate-800 rounded-3xl shadow-2xl overflow-hidden p-6 space-y-5">
            ...
            <div className="text-center space-y-2">
              ...
              <h2 className="text-lg font-bold text-white">Transfer Successfully Processed</h2>
              <p className="text-xs text-slate-400">Digital confirmation receipt issued instantly</p>
            </div>

            <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 space-y-3 text-xs">
              <div className="flex justify-between border-b border-slate-800/80 pb-2">
                <span className="text-slate-400">Transaction Ref:</span>
                <span className="font-mono text-blue-400 font-bold">{statusState.receipt.txId}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Date & Time:</span>
                <span className="text-slate-200">{statusState.receipt.timestamp}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">From Account:</span>
                <span className="text-slate-200">{statusState.receipt.fromAccountName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Recipient / To:</span>
                <span className="text-slate-200 font-semibold">{statusState.receipt.toName}</span>
              </div>
              <div className="flex justify-between border-t border-slate-800/80 pt-2 text-sm">
                <span className="text-slate-300 font-bold">Amount Transferred:</span>
                <span className="text-emerald-400 font-black">${statusState.receipt.amount.toFixed(2)} USD</span>
              </div>
            </div>
