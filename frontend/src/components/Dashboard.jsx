...
export default function Dashboard({ setActiveTab, onOpenTransfer, onOpenInsuranceModal, onOpenClaimModal }) {
  ...
  const getPolicyIcon = (type) => {
    switch (type) {
      case 'Health': return <HeartPulse className="w-4 h-4 text-emerald-400" />;
      case 'Auto': return <Car className="w-4 h-4 text-blue-400" />;
      case 'Home': return <Home className="w-4 h-4 text-purple-400" />;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="relative rounded-3xl bg-gradient-to-r from-blue-950/80 via-slate-900 to-indigo-950/90 border border-slate-800 p-6 sm:p-8 overflow-hidden shadow-xl">
        ...
        <div className="absolute bottom-0 right-1/4 -mb-10 w-48 h-48 bg-emerald-500/10 rounded-full blur-2xl pointer-events-none"></div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-blue-500/10 text-blue-400 border border-blue-500/20 flex items-center gap-1">
                <Sparkles className="w-3 h-3 text-blue-400" /> Apex Preferred Banking
              </span>
            ...
            <p className="text-xs sm:text-sm text-slate-400 mt-1 max-w-xl">
              Your overall net liquidity stands at <strong className="text-white">${totalLiquidity.toLocaleString('en-US', { minimumFractionDigits: 2 })}</strong> with {policies.length} active insurance protections.
            </p>
          </div>

          {/* Credit & Risk Pill */}
          <div className="flex items-center gap-3 bg-slate-900/80 p-3.5 rounded-2xl border border-slate-800 backdrop-blur-md">
            <div className="text-center px-3 border-r border-slate-800">
              <p className="text-[10px] text-slate-400 uppercase font-semibold">FICO® Score</p>
              <p className="text-xl font-black text-emerald-400">{user.creditScore}</p>
              <p className="text-[9px] text-emerald-300 font-medium">Excellent</p>
            </div>
            <div className="text-center px-3">
              <p className="text-[10px] text-slate-400 uppercase font-semibold">Risk Rating</p>
              <p className="text-sm font-bold text-blue-400 mt-1">{user.riskRating}</p>
              <p className="text-[9px] text-slate-400">Tier 1 Approved</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {accounts.map(acc => (
          <div
            key={acc.id}
            className="p-5 rounded-2xl bg-[#131b2e] border border-slate-800 hover:border-slate-700 transition glass-card-hover flex flex-col justify-between"
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400">{acc.accountNumber}</p>
                  </div>
                </div>
      ...
      {/* Quick Actions Strip */}
      <div className="p-4 rounded-2xl bg-[#131b2e] border border-slate-800 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            <FileText className="w-4 h-4 text-amber-400" /> File Claim
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-5 rounded-2xl bg-[#131b2e] border border-slate-800 space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-blue-400" /> Recent Transactions
              </h2>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-slate-400 mt-0.5">
                      ...
                      <span>•</span>
                      <span className="px-1.5 py-0.2 rounded bg-slate-800 text-slate-300 font-medium">{tx.category}</span>
                    </div>
                  </div>
                </div>

                <div className="text-right">
                  <p className={`font-bold ${tx.type === 'credit' ? 'text-emerald-400' : 'text-slate-200'}`}>
                    {tx.type === 'credit' ? '+' : '-'}${tx.amount.toFixed(2)}
                  </p>
                  <span className="text-[10px] text-emerald-400 font-medium">{tx.status}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={() => setActiveTab('transfers')}
            className="w-full text-center text-xs text-blue-400 font-medium hover:text-blue-300 pt-2 flex items-center justify-center gap-1"
          >
        ...
        <div className="space-y-5">
          ...
          <div className="p-5 rounded-2xl bg-[#131b2e] border border-slate-800 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-emerald-400" /> Active Policies
              </h2>
              <button
            ...
            <div className="space-y-3">
              {policies.map(p => (
                <div key={p.id} className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 hover:border-slate-700 transition">
            ...
            <button
              onClick={onOpenInsuranceModal}
              className="w-full py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-semibold text-xs transition shadow-md flex items-center justify-center gap-1.5"
            >
          ...
          <div className="p-5 rounded-2xl bg-[#131b2e] border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-bold text-white flex items-center gap-2">
                <FileText className="w-4 h-4 text-amber-400" /> Active Claims Tracker
              </h2>
              <button
            ...
            {claims.map(claim => (
              <div key={claim.id} className="p-3 rounded-xl bg-slate-900/80 border border-slate-800 text-xs space-y-2">
